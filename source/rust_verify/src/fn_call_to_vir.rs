...
use crate::context::{AtomicallyCtxt, BodyCtxt, HeaderSetting, PendingTrackedArg};
...
use crate::rust_to_vir_expr::{
    ...
    expr_to_vir_place, extract_array, extract_tuple, get_fn_path, mk_ty_clip, pat_to_var,
};
...
use rustc_hir::{Block, BlockCheckMode, Expr, ExprKind, QPath, StmtKind};
...
pub(crate) fn is_with_shim_fn(tcx: rustc_middle::ty::TyCtxt<'_>, def_id: DefId) -> bool {
    crate::attributes::is_with_shim(with_shim_attrs(tcx, def_id), None).unwrap_or(false)
}

pub(crate) fn is_with_arg_shim_fn(tcx: rustc_middle::ty::TyCtxt<'_>, def_id: DefId) -> bool {
    crate::attributes::is_with_arg_shim(with_shim_attrs(tcx, def_id))
}

fn with_shim_attrs<'tcx>(
...
fn find_original_fn_for_shim(
    ...
    let shim_name_sym = tcx.item_name(shim_def_id);
    let shim_name = shim_name_sym.as_str();
    let original_name = shim_name
        .strip_prefix(WITH_RET_SHIM_PREFIX)
        .or_else(|| shim_name.strip_prefix(WITH_SHIM_PREFIX))?;
    ...
    match tcx.def_kind(parent_def_id) {
        rustc_hir::def::DefKind::Trait | rustc_hir::def::DefKind::Impl { .. } => tcx
            .associated_items(parent_def_id)
            .filter_by_name_unhygienic(original_sym)
            .find(|assoc| matches!(assoc.kind, rustc_middle::ty::AssocKind::Fn { .. }))
            .map(|assoc| assoc.def_id),
        _ => {
            if let Some(local_parent) = parent_def_id.as_local() {
                let krate = tcx.hir_crate(());
                let rustc_hir::MaybeOwner::Owner(owner_info) = krate.owner(tcx, local_parent)
                else {
                    return None;
                };
                let item_ids = match owner_info.node() {
                    rustc_hir::OwnerNode::Crate(c) => c.item_ids,
                    rustc_hir::OwnerNode::Item(item) => match &item.kind {
                        rustc_hir::ItemKind::Mod(_, mod_items) => mod_items.item_ids,
                        _ => return None,
                    },
                    _ => return None,
                };
                item_ids
                    .iter()
                    .map(|item_id| item_id.owner_id.def_id.to_def_id())
                    .find(|def_id| is_fn(*def_id) && tcx.item_name(*def_id) == original_sym)
            } else {
                tcx.module_children(parent_def_id)
                    .iter()
...
pub(crate) fn fn_call_to_vir<'tcx>(
    ...
    // Handle shim calls: _VERUS_WITH_f(regular_args, extras) → f(regular_args) with extras
    if is_with_arg_shim_fn(tcx, f) {
        return err_span(
            expr.span,
            "`with` conformance companions are generated by Verus and cannot be called directly",
        );
    }
    if is_with_shim_fn(tcx, f) {
        let Some(original_f) = find_original_fn_for_shim(tcx, f) else {
            return err_span(
                expr.span,
                "`with` shims are generated by Verus and cannot be called directly; call the function they belong to and pass the extras with proof_with()",
            );
        };
        let shim_params = tcx.fn_sig(f).skip_binder().skip_binder().inputs().len();
        let original_params = tcx.fn_sig(original_f).skip_binder().skip_binder().inputs().len();
        if args.len() < original_params || shim_params < original_params {
            return err_span(expr.span, "internal error: malformed `with` shim call");
        }
        let (regular_args, extra_args) = args.split_at(original_params);

        // The extras are ghost/tracked: lowering them in exec mode would attach
        // arithmetic proof obligations to ghost code and mis-record erasure.
        let bctx_ghost = &BodyCtxt { in_ghost: true, ..bctx.clone() };
        let mut extras: Vec<PendingTrackedArg> = Vec::new();
        for e in extra_args {
            let vir_expr = expr_to_vir_consume(bctx_ghost, e)?;
            extras.push(PendingTrackedArg { expr: vir_expr });
        }

        let with_ret = tcx.item_name(f).as_str().starts_with(WITH_RET_SHIM_PREFIX);
        *bctx.pending_tracked_args.borrow_mut() =
            Some(crate::context::PendingWith { args: extras, with_ret });

        // Recursively call with the original function and regular args
        return fn_call_to_vir(
            bctx,
            expr,
            original_f,
            node_substs,
...
fn fn_call_or_assoc_const_to_vir<'tcx>(
    ...
    // Take this call's extras before lowering its arguments: an argument may
    // itself be a `proof_with` call, which sets the same slot, and would
    // otherwise consume or clobber ours.
    let my_pending = bctx.pending_tracked_args.borrow_mut().take();
    ...
    // Consume the extras produced by the `with` shim rewrite, if any. rustc has
    // already checked their types, modes, borrows and regions against the shim's
    // parameter list; what remains is to append them to the VIR call.
    ...
    let vir_args = {
        ...
        if extra_params.is_some() || has_ret_params {
            ...
            let (pending, with_ret) = match my_pending {
                Some(p) => (p.args, p.with_ret),
                None => {
                    if extra_count > 0 {
                        return err_span(
                            expr.span,
            ...
            // A VIR call whose argument count disagrees with the callee's parameter
            // count trips an assertion deep in VIR, so report it here instead.
            if pending.len() != extra_count {
                return err_span(
                    expr.span,
                    format!(
                        "this function takes {} extra tracked/ghost argument(s) via proof_with(), but {} were supplied",
                        extra_count,
                        pending.len()
                    ),
                );
            ...
            args.extend(exprs);
            call_site_with_ret = with_ret;
        } else if my_pending.is_some() {
            return err_span(
                expr.span,
                "this function does not take extra tracked/ghost arguments via proof_with()",
            );
        }
        args
    };
    ...
    match verus_item {
        ...
        VerusItem::Expr(expr_item) => match expr_item {
            ...
            ExprItem::Old => {
                ...
                let arg = expr_to_vir_consume(bctx, &args[0])?;
            ...
            ExprItem::ArrayIndex => {
                ...
                match &expr.kind {
                    ExprKind::Call(_, args) if args.len() == 2 => {
                        ...
                        let arg0 = expr_to_vir_consume(bctx, arg0).expect(
                            "invalid parameter for verus_builtin::array_index at arg0, arg0 must be self",
                        );
                        let arg1 = &args[1];
                        let arg1 = expr_to_vir_consume(bctx, arg1)
            ...
            ExprItem::ClosureToFnSpec | ExprItem::ClosureToFnProof => {
                ...
                if let ExprKind::Closure(..) = &args[0].kind {
                    ...
                    closure_to_vir(bctx, &args[0], expr_typ()?, is_spec_fn, proof_fn_modes)
                } else {
            ...
            ExprItem::IsVariant => {
                record_spec_fn(bctx, expr);
                assert!(args.len() == 2);
                let adt_arg = expr_to_vir_consume(bctx, &args[0])?;
        ...
        VerusItem::Assert(assert_item) => {
            ...
            match assert_item {
                AssertItem::Assert => {
                    ...
                    let exp = expr_to_vir_consume(bctx, &args[0])?;
                ...
                AssertItem::AssertBy => {
                    ...
                    let ensure = expr_to_vir_consume(bctx, &args[0])?;
                    let proof = expr_to_vir_consume(bctx, &args[1])?;
                ...
                AssertItem::AssertNonlinearBy | AssertItem::AssertBitvectorBy => {
                    ...
                    let mut vir_expr = expr_to_vir_consume(&bctx, &args[0])?;
                ...
                AssertItem::AssertBitVector => {
                    let vir_expr = expr_to_vir_consume(bctx, &args[0])?;
        ...
        VerusItem::WithTriggers => {
            ...
            let triggers_tuples = expr_to_vir_consume(bctx, args[0])?;
            let body = expr_to_vir_consume(bctx, args[1])?;
        ...
        VerusItem::UnaryOp(UnaryOpItem::SpecCastReal) => {
            ...
            let source_vir0 = expr_to_vir(bctx, &args[0])?;
        ...
        | VerusItem::CompilableOpr(CompilableOprItem::TrackedBorrowMut) => {
            ...
            let vir_arg = expr_to_vir_place(bctx, &args[0])?;
        ...
        VerusItem::BuiltinFunction(
            ...
            let vir_args = args
                .iter()
                .map(|arg| expr_to_vir_consume(bctx, &arg))
        ...
        VerusItem::ProofWith | VerusItem::ProofWithRet => {
            // The pre-typeck HIR pass (hir_proof_with_rewrite) replaces every
            // `proof_with`/`proof_with_ret` marker with a call to the callee's
            // `with` shim, so that rustc checks the extras. Reaching this point
            // means the rewrite did not happen and nothing has been checked.
            return err_span(
                expr.span,
                "internal error: this `proof_with` call was not rewritten, so its extra ghost/tracked arguments were never checked",
            );
        }
...
fn extract_choose<'tcx>(
    ...
    match &expr.kind {
        ExprKind::Closure(closure) => {
            ...
            let cond = expr_to_vir_consume(bctx, cond_expr)?;
            let body = if tuple {
...
fn mk_two_vir_args<'tcx>(
    ...
    let e0 = expr_to_vir_consume(bctx, &args[0])?;
    let e1 = expr_to_vir_consume(bctx, &args[1])?;
    Ok((e0, e1))
}
