...
fn fn_call_or_assoc_const_to_vir<'tcx>(
    ...
    // While lowering the arguments, a nested call must not consume the pending
    // `proof_with()` arguments that belong to this call.
    *bctx.in_args_depth.borrow_mut() += 1;
    let lowered_args = (|| -> Result<_, VirErr> {
        Ok(match args.as_deref() {
    ...
    *bctx.in_args_depth.borrow_mut() -= 1;
    let (vir_args, call_body) = lowered_args?;
