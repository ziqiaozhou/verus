...
fn add_fndef_axioms_to_function(
    ...
    let (closure_enss, default_enss) = function.x.ensure.clone();
    let mut closure_enss = (*closure_enss).clone();
    let mut default_enss = (*default_enss).clone();
    if inherit {
    ...
    let closure_enss = Arc::new(closure_enss);
    let default_enss = Arc::new(default_enss);

    let mut enss_list: Vec<(BuiltinSpecFun, Arc<Vec<Expr>>, bool)> = Vec::new();
